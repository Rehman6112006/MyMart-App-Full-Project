...
import { getVendorOrders, updateVendorOrderStatus, markVendorPaymentReceived } from '../services/api';
                ...
                {filtered.map((o, i) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      {o.user_email && <p className="text-xs text-gray-400 truncate max-w-[150px]">{o.user_email}</p>}
                    </td>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900">
                      Order <span className="font-mono">{formatOrderId(selectedOrder)}</span>
                    </h2>
                    <p className="text-xs text-gray-400">{selectedOrder.store_name || 'My Store'}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <p><span className="text-gray-500">Email:</span> <span className="text-gray-700">{selectedOrder.user_email || '—'}</span></p>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-4 gap-3">
                    <div className="bg-emerald-50 rounded-xl p-3 text-center">
                      ...
                      <p className="font-semibold text-gray-900">{formatCurrency(selectedOrder.subtotal || selectedOrder.total_amount || 0)}</p>
                    </div>
                    <div className="bg-blue-50 rounded-xl p-3 text-center">
                      <p className="text-xs text-gray-500">Delivery</p>
                      <p className="font-semibold text-gray-900">{formatCurrency(selectedOrder.delivery_charge || selectedOrder.shipping_cost || 0)}</p>
                    </div>
                    ...
                    <div className="bg-emerald-50 rounded-xl p-3 text-center">
                      ...
                      <p className="font-semibold text-emerald-600">{formatCurrency(selectedOrder.total_amount || selectedOrder.total || 0)}</p>
                    </div>
                  </div>
                    <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {(selectedOrder.payment_method || '').toLowerCase() === 'cod' && selectedOrder.payment_status !== 'paid' && (selectedOrder.order_status || selectedOrder.status) === 'delivered' && (
                          <button onClick={() => { if (window.confirm('Mark payment as received?')) { markVendorPaymentReceived(selectedOrder.id).then(() => { fetchOrders(); setSelectedOrder(prev => prev ? { ...prev, payment_status: 'paid' } : null); }); } }}
