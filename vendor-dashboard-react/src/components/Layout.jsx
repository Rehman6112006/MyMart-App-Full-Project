...
import {
  LayoutDashboard, ShoppingBag, Package, Store,
  Star, Wallet, LogOut, Menu, X, ChevronLeft, ChevronRight,
  TrendingUp, FileText, Tag, Bell, UserCircle
} from 'lucide-react';

const menuItems = [
  { path: '/', label: 'Dashboard', icon: LayoutDashboard, color: 'from-emerald-500 to-green-600' },
  { path: '/products', label: 'Products', icon: Package, color: 'from-blue-500 to-cyan-600' },
  { path: '/orders', label: 'Orders', icon: ShoppingBag, color: 'from-orange-500 to-red-500' },
  { path: '/store', label: 'My Store', icon: Store, color: 'from-teal-500 to-emerald-600' },
  ...
  { path: '/profile', label: 'Profile', icon: UserCircle, color: 'from-cyan-500 to-blue-600' },
];
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30">
      <SkipToContent />
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-gradient-to-r from-emerald-600 to-green-600 flex items-center px-4 z-30 shadow-lg">
        ...
        <div className="ml-4 flex items-center gap-3">
          ...
          <span className="font-bold text-xl text-white">MyMart Vendor</span>
        </div>
      </div>
      ...
      <motion.aside
        ...
        className={`fixed top-0 left-0 h-full z-40 shadow-2xl ${
          mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="h-full bg-gradient-to-b from-emerald-600/90 via-green-600/90 to-emerald-700/90 backdrop-blur-xl">
              ...
              {sidebarOpen ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-emerald-200 font-medium">Vendor Portal</p>
                  </div>
                </motion.div>
              ) : (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center mx-auto backdrop-blur-sm">
                  <ShoppingBag size={22} className="text-white" />
                </motion.div>
              )}
            </AnimatePresence>
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden lg:flex p-2 hover:bg-white/10 rounded-lg">
              {sidebarOpen ? <ChevronLeft size={20} className="text-white/80" /> : <ChevronRight size={20} className="text-white/80" />}
            </button>
          </div>

          <button onClick={() => setMobileOpen(false)} className="lg:hidden absolute top-4 right-4 p-2 hover:bg-white/10 rounded-lg">
              ...
              return (
                <motion.button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 ${
                    isActive ? 'bg-white/20 backdrop-blur-sm shadow-lg shadow-black/10' : 'hover:bg-white/10'
                  }`}
                >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/50 z-30 lg:hidden backdrop-blur-sm" onClick={() => setMobileOpen(false)} />
        )}
      </AnimatePresence>

      <motion.main id="main-content" initial={false} animate={{ marginLeft: sidebarOpen ? 260 : 80 }}
        transition={{ duration: 0.3, ease: 'easeInOut' }}
        className="min-h-screen pt-16 lg:pt-0">
        <motion.div key={location.pathname} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: 'easeOut' }} className="p-6">{children}</motion.div>
      </motion.main>
    </div>
  );
}